...
async function handleUpload(request, env) {
  ...
  await env.PORTFOLIO_IMAGES.put(key, file.stream(), {
    ...
    customMetadata: { title: title.slice(0, 120), category }
  });
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/works" && request.method === "GET") return await handleWorks(env);
      if (url.pathname.startsWith("/files/") && request.method === "GET") return await handleFile(request, env);
